...
//        QMessageBox::critical(nullptr, QString::fromUtf8("OneMaker MV: FATAL ERROR"), QString::fromUtf8("hijack.rcc not found"), QMessageBox::StandardButton::Abort);
        QTextStream(stderr) << "OneMaker MV: FATAL ERROR: hijack.rcc not found" << endl;
    }
...
/*/
    ...
    QString resultPath = dest + "/_hijack_root" + file;
    // std::wstring_convert<std::codecvt_utf8<wchar_t>> convertron;
    // std::string convertedPath = convertron.to_bytes(resultPath);
    // std::stringstream ss;
    // ss << convertedPath;

    QFile resFile (resultPath);
    if (resFile.exists()) { // TODO: directory check
        //QMessageBox::information(nullptr, QString::fromUtf8("OneMaker MV: Redirecting..."), resultPath);
        //ss << " :::: REDIRECTED :::::";

        if (resFile.open(QFile::OpenModeFlag::ReadOnly)) {
            qint64 fsize = resFile.size();
            data = (uchar*)malloc(fsize);
            qint64 bytes_read = resFile.read((char*)data, fsize);
            resFile.close();
