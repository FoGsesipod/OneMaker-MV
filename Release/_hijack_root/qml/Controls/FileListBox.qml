...
ListBox {
    ...
    readonly property string currentPrefix: getCurrentMember("prefix", "")
    readonly property string currentPath: getCurrentMember("filePath", "")

    readonly property real currentScale: getCurrentMember("imageScale", 0)
    readonly property int currentFrameWidth: getCurrentMember("frameWidth", 0)
    readonly property int currentFrameHeight: getCurrentMember("frameHeight", 0)
    readonly property int currentMaxColumns: getCurrentMember("maxColumns", 0)
    ...
    style: CustomScrollViewStyle {
        ...
        property Component itemDelegate: Item {
            ...
            Image {
                ...
                source: ImagePack.obtainImagePack() + "warning.png" // [OneMaker MV] - Obtain Image Pack
    ...
    function makeFileExtensionIssue(fileObj) {
        var issue = {
            ...
            buttons: [{
                text: qsTr("Rename"),
                title: qsTr("Rename the file"),
                hint: qsTr("Change the extension name to “%1”.").arg(fileObj.targetSuffix),
                onClicked: function() { console.log("Not Implemented"); }
            }]
            */
        };
        return issue;
    ...
    function refresh() {
        var baseNames = [];
        var list = [];
        var lowerAllowedSuffixes = null;
        var allIssues = [];
        if (allowedSuffixes) {
            lowerAllowedSuffixes = allowedSuffixes.map(function(i) { return i.toLowerCase() });
        }
        for (var i = 0; i < folderListModel.count; i++) {
            var suffix = folderListModel.get(i, "fileSuffix");
            var targetSuffix = suffix;
            if (allowedSuffixes) {
                var suffixIndex = lowerAllowedSuffixes.indexOf(suffix.toLowerCase());
                if (suffixIndex < 0) continue;
                targetSuffix = allowedSuffixes[suffixIndex];
            }
            var fileName = folderListModel.get(i, "fileName");
            var dot = fileName.lastIndexOf('.');
            var baseName = dot > 0 ? fileName.substring(0, dot) : fileName;
            var noPrefixName = baseName.replace(prefixRegexp, '');
            var obj = {};
            obj.fileName = fileName;
            obj.fileBaseName = baseName;
            obj.filePath = folder + "/" + encodeURIComponent(fileName);
            obj.prefix = baseName.replace(noPrefixName, '');
            obj.nodeName = noPrefixName.replace(/\d+$/, '');
            obj.number = Number(noPrefixName.replace(obj.nodeName, ''));
            obj.suffix = suffix;
            obj.targetSuffix = targetSuffix;
            var issues = [];
            if (obj.targetSuffix !== obj.suffix) {
                issues.push(makeFileExtensionIssue(obj));
            }
            obj.hasIssues = false;
            obj.issuesIndex = -1;
            if (issues.length > 0) {
        ...
        list.sort(compareFileNameObject);
        ...
        if (additionalItems) {
            for (i = 0; i < additionalItems.length; i++) {
                sortedListModel.append(additionalItems[i]);
            }
        }
        updated();
    }
    ...
    function compareFileNameObject(a, b) {
