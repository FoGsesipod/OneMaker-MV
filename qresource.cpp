...
QResourcePrivate::load(const QString &file)
{
    ...
    //HANDLE logFileWriter = CreateFileW((LPCWSTR)L"hijacker.log", FILE_APPEND_DATA, FILE_SHARE_READ, NULL, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
    
    std::wstringstream pathwss;

    pathwss << dest;
    pathwss << "/_hijack_root";
    pathwss << file.toStdWString();
    ...
    if ((dwAttrib != INVALID_FILE_ATTRIBUTES && 
        !(dwAttrib & FILE_ATTRIBUTE_DIRECTORY))) {
        //MessageBox(NULL, resultPath.c_str(), (LPCWSTR)L"Will Redirect:", 0);
        //ss << " :::: REDIRECTED :::::";

        HANDLE h = CreateFile(resultPath.c_str(), GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
        DWORD fsize = GetFileSize(h, NULL);
        DWORD bytes_read = 0;
        data = (uchar*)malloc(fsize);
        ReadFile(h, (LPVOID)data, fsize, &bytes_read, NULL);
        CloseHandle(h);
